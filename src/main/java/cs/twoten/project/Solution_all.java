...
public class Solution_all {
    ...
    index.Search(parts[1]);   // ← search only, do not delete
    break;
    ...
    static class Index {
        ...
        int Balance(AVLNode key) {
            return (key == null) ? 0 : (Height(key.right) - Height(key.left));
        }
        ...
        AVLNode Successor(AVLNode r) {
            if (r.left != null) return Successor(r.left);
            return r;
        }
